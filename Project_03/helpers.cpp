...
vector< vector<float> > normalize(vector< vector <float> > grid)
{
    ...
    // first compute total sum of vector's element
    ...
    const float scaler = 1.0 / sum;
    
    // iterating all elements divide by total sum
    for (int i=0; i < height; i++)
    {
        for (int j=0; j < width; j++)
        {
            grid[i][j] = grid[i][j] * scaler;
        }
    }
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring)
{
    ...
    // initialize 3x3 window parameters
    ...
    // 3x3 2D vector
    const float window[3][3] = {{corner_prob, adjacent_prob, corner_prob},
        {adjacent_prob, center_prob, adjacent_prob},
        {corner_prob, adjacent_prob, corner_prob}};
    
    // initerating all grid elements using 3x3 blurring window
    for (int i=0; i < height; i++)
    {
        for (int j=0; j < width; j++)
        {
